...
\maketitle
    ...
    \section{反復過程とその極限}
        ...
        (つまり, $v_n = u_{n+1} - u_{n}$である.)
        \vskip \baselineskip
        まず, Theorem \ref{thm:twi}のもとで, Lemma \ref{lem:8}, Lemma \ref{lem:10}を繰り返し適用できると仮定したときに,
        収束先が存在して$E(u) \equiv 0$となることを示したい.

        Lemma \ref{lem:8}の\eqref{eq:18}を$u = u_n, r = r_n, N = 2 N_0$として適用すれば, 
        $\epsilon_n := |E(u_n)|_{r_n}$として, 
        \begin{equation} \label{eq:32}
            |v_n|_{r_{n+1}} \leq \frac{c_1}{(r_n - r_{n-1})^{2 \tau}} \epsilon_n
        \end{equation}
        であり, 
        \begin{equation} \label{eq:33}
            |(v_n)_{\theta}|_{r_{n+1}} \leq \frac{c_1}{(r_n - r_{n-1})^{2 \tau + 1}} \epsilon_n
        \end{equation}
        である.
        また, $r_n - r_{n-1} = 2^{-(n+1)}(r_0 - r_{\infty})$であるので,
        $\displaystyle a := 2^{4 \tau}, c_7 := c_6 \frac{2^{4 \tau}}{r_0 - r_{\infty}}$とすれば,
        Lemma \ref{lem:10}から,
        \begin{equation} \label{eq:34}
            \epsilon_{n+1} \leq \frac{c_6 \epsilon_n^2}{(r_n - r_{n-1})^{2 \tau + 1}} \epsilon_n = c_7 a^n \epsilon_n^2.
        \end{equation}

        問題は. $n$を大きくしたときに$a^n$の発散するのと$\epsilon_n^2$が$0$に近づくのはどちらが早いかということである.
        これを調べるには$\{\eta_n\}$を
        \begin{equation} \label{eq:35}
            \eta_n := a^{n+1} c_7 \epsilon_n
        \end{equation}
        で定めればよく, 実際\eqref{eq:34}から
        \[
            \eta_{n+1} = a^{n+2} c_7 \epsilon_{n+1} \leq a^{2n+2} c_7^2 \epsilon_{n}^2 = \eta_n^2
        \]
        であるから, $\eta_0 = a c_7 \epsilon_0 < 1$の下で$\eta_n \leq \eta_0^{2^n} \rightarrow 0$, つまり$\epsilon_n \leq \eta_n \rightarrow 0$がわかる.
        \eqref{eq:32}から$|v_n|_{r_{\infty}} \leq |v_n|_{r_{n+1}} \rightarrow 0$であるが,
        特にこれは指数関数より早く収束するので, 無限和$u_0 + \sum_{k = 0}^{\infty} v_k$は絶対収束し, $u_\infty \in W_{r_{\infty}}$となる.
        これにより, $\displaystyle E(u_\infty) = \lim_{n \rightarrow \infty} E(u_n) = 0$がわかる. 
        \vskip \baselineskip

        次に, $u = u_n, r = r_n, N = 2 N_0$としたときに
        Lemma \ref{lem:8}, Lemma \ref{lem:10}の仮定\eqref{eq:16} \eqref{eq:17}を満たすことを示す.

        これは, 
        \begin{equation} \label{eq:36}
            |u_n - u_0|_{r_n} < \frac{R}{2}
        \end{equation}
        および
        \begin{equation} \label{eq:37}
            |(u_n - u_0)_{\theta}|_{r_n} < \frac{1}{2N_0}
        \end{equation}
        を示せば十分である. 実際, \eqref{eq:36}および\eqref{eq:7}から\eqref{eq:16}で$R$を$R/2$に置き換えたものが満たされ, \eqref{eq:27}および\eqref{eq:8}から,
        \[
            |(u_n)_{\theta}|_{r_n} \leq |(u_n - u_0)_{\theta}|_{r_n} + |(u_0)_{\theta}|_{r_n} < \frac{1 + 2 N_0^2}{2 N_0} < 2 N_0
        \]
        であるから, \eqref{eq:17}で$N = 2 N_0$としたものが満たされる.
