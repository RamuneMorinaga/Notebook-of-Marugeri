...
\maketitle
    ...
    \section{ハミルトン方程式に関するネーターの定理}
        ...
        \begin{example}[ラプラス-ルンゲ-レンツベクトル]
            ...
            ハミルトニアン$H$, 角運動量$G$とは別の第一積分であるラプラス-ルンゲ-レンツベクトル$\tilde{D}$が存在する:
            これは局所座標の下で第$i$成分が
            \[
                (\tilde{D}(x, y))_i = - (x \cdot y) y_i + |y|^2 x_i - \frac{x_i}{|x|}
            \]
            で表されるものである.
    ...
    \vskip \baselineskip
            ...
            \vskip \baselineskip
            以上から, $T^{+} \mathbb{S}^{n}$上のハミルトンベクトル場$X_{\hat{f}}$の$\mathbb{R}^{2(n+1)}$上への押し出し$\hat{X}_{\hat{f}} := i_{*} X_{f}$は
            \[
                \hat{X}_{\hat{f}} = \langle \nabla_{\bar{p}} f,  \left(\frac{\partial}{\partial \bar{q}}\right)\rangle - \langle \nabla_{\bar{q}} f,  \left(\frac{\partial}{\partial \bar{p}}\right)\rangle + \langle \frac{\partial f}{\partial \bar{p}},  \Gamma \left(\frac{\partial}{\partial \bar{p}}\right)\rangle
            \]
            と書ける.
            ただし, $\nabla_{\bar{p}}, \nabla_{\bar{q}}, \Gamma$は以下で定める:
            \[
                \nabla_{\bar{p}} f := \frac{\partial f}{\partial \bar{p}} - \langle \frac{\partial f}{\partial \bar{p}}, \bar{q} \rangle \bar{q}, \ 
                \nabla_{\bar{q}} f := \frac{\partial f}{\partial \bar{q}} - \langle \frac{\partial f}{\partial \bar{q}}, \bar{q} \rangle \bar{q}, \ 
                \left(\Gamma \left(\frac{\partial}{\partial \bar{p}}\right) \right)_{i} := \sum_{j} (q_i p_j - p_i q_j) \frac{\partial}{\partial p_j}.
            \]

            さて, $f = \norm{\bar{p}}$, つまり$\hat{f} = K_1$となる場合について考えれば,
            \[
                (\nabla_p f)_{i} = \frac{p_i}{\norm{\bar{p}}} - \frac{\bar{p} \cdot \bar{q}}{\norm{\bar{p}}} q_i =  \frac{p_i}{\norm{\bar{p}}}, \ (\nabla_{\bar{q}} f)_{i} = 0, \ \langle \frac{\partial f}{\partial \bar{p}},  \Gamma \left(\frac{\partial}{\partial \bar{p}}\right)\rangle = \left(\sum_{j} (q_j p_i - p_j q_i) \frac{p_j}{\norm{\bar{p}}} \right)\frac{\partial}{\partial p_i} = - q_i \frac{\partial}{\partial p_i}.
            \]
            であるから, 
            \[
                \hat{X}_{\hat{f}} = \sum_{i = 0}^{n} \frac{p_i}{\norm{\bar{p}}} \frac{\partial}{\partial q_i} - q_i \frac{\partial}{\partial p_i}.
            \]
            となる. 
            特に, $\hat{f} = K_1 = 1$の曲面$T^{1} \mathbb{S}^n$上で,
            \[
